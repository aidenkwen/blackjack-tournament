...
const SeatingAssignmentPage = ({
  ...
  const toggleConflictTable = (tableNumber) => { setConflictTables(prev => { const newSet = new Set(prev); if (newSet.has(tableNumber)) { newSet.delete(tableNumber); } else { newSet.add(tableNumber); } return newSet; }); };
  ...
  const confirmAssignment = () => { if (!selectedSeat) { alert('Please select a seat first.'); return; } setConfirming(true); const pendingRegKeys = new Set(pendingRegistration.registrations.map(p => `${p.eventType}-${p.isRebuy}-${p.isMulligan}`)); const registrationsWithoutOldEntries = registrations.filter(reg => !(reg.playerAccountNumber === lastRegisteredPlayer.playerAccountNumber && pendingRegKeys.has(`${reg.eventType}-${reg.isRebuy}-${reg.isMulligan}`))); const newRegistrationsWithSeats = pendingRegistration.registrations.map(pendingReg => ({ ...pendingReg, round: lastRegisteredPlayer.round, timeSlot: lastRegisteredPlayer.timeSlot, tableNumber: selectedSeat.table, seatNumber: selectedSeat.seat })); const finalRegistrations = [...registrationsWithoutOldEntries, ...newRegistrationsWithSeats]; setRegistrations(finalRegistrations); alert(`${lastRegisteredPlayer.firstName} ${lastRegisteredPlayer.lastName} assigned to Table ${selectedSeat.table}, Seat ${selectedSeat.seat}`); setPendingRegistration(null); setCurrentPage(1); setConfirming(false); };

  return (
    <div className="container">
      <button onClick={() => setCurrentPage(1)} className="link-back link-back-block" disabled={confirming}> {'<'} Back to Registration </button>
      <div className="page-header"> <h1 className="page-title">{selectedEvent} / Player Seating</h1> <p className="page-subtitle">{currentRound?.name} - Time Slot {lastRegisteredPlayer.timeSlot}</p> </div>
      ...
      <div style={{
        ...
        {/* Left Side: PlayerInfoCard for consistent styling */}
        <div>
          <PlayerInfoCard
            currentPlayer={pendingRegistration.player}
            activeTab={pendingRegistration.activeTab}
            selectedRound={lastRegisteredPlayer.round}
          />
        </div>

        {/* Right Side: Seat Preferences, styled to perfectly match the image */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '1rem', color: '#000000', fontWeight: 400 }}>
            Seat Preferences:
          </span>
          {[1, 2, 3, 4, 5, 6].map(seatNum => (
            <label key={seatNum} style={{
              display: 'flex',
              alignItems: 'center',
              gap: '4px',
              cursor: 'pointer',
      ...
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}> <button onClick={assignRandomSeat} className="btn btn-primary" disabled={confirming}>{seatPreferences.length > 0 ? `Assign Random Seat (${seatPreferences.join(', ')})` : 'Assign Random Seat'}</button> <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}> <button onClick={confirmAssignment} className="btn btn-success" disabled={confirming || !selectedSeat} style={{ opacity: (!selectedSeat || confirming) ? 0.5 : 1 }}>{confirming ? 'Confirming...' : selectedSeat ? `Confirm: T${selectedSeat.table}, S${selectedSeat.seat}` : 'Confirm Seat'}</button> <button onClick={() => setSelectedSeat(null)} className="btn btn-secondary" disabled={confirming || !selectedSeat} style={{ opacity: (!selectedSeat || confirming) ? 0.5 : 1 }}>Clear</button> </div> </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        {[1, 2, 3, 4, 5, 6].map(tableNumber => (
          <div key={tableNumber} style={{ border: '1px solid #ddd', borderRadius: '8px', padding: '16px', backgroundColor: isTableUnavailable(tableNumber) ? '#e0e0e0' : '#f2f2f2' }}>
            ...
            {isTableDisabled(tableNumber) ? (<div style={{ height: '80px', backgroundColor: '#666666', border: '2px solid #ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ffffff', fontSize: '0.9rem', fontWeight: 'bold' }}>{lastRegisteredPlayer.round === 'semifinals' && tableNumber === 6 ? 'Table Disabled (Semifinals)' : 'Table Disabled'}</div>) : isTableConflicted(tableNumber) ? (<div style={{ height: '80px', backgroundColor: '#666666', border: '2px solid #ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ffffff', fontSize: '0.9rem', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => toggleConflictTable(tableNumber)} title="Click to remove conflict restriction">Conflict of Interest (Click to Remove)</div>) : (
