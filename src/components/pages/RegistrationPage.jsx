...
const RegistrationPage = ({
  ...
  useEffect(() => {
    if (activeTab === 'registration' && !selectedTimeSlot) setSelectedTimeSlot(1);
    else if (activeTab === 'post-registration' && selectedRound && !selectedTimeSlot) setSelectedTimeSlot(1);
  }, [activeTab, selectedRound, selectedTimeSlot]);
  ...
  const isRegisteredRound1 = acct => {
    ...
    return currentTournamentRegistrations.some(r => 
      normalizeAccount(r.playerAccountNumber) === normalizedAcct && r.round === 'round1'
    );
  };
  ...
  const isRegisteredRebuy1 = acct => {
    const normalizedAcct = normalizeAccount(acct);
    return currentTournamentRegistrations.some(r => 
      normalizeAccount(r.playerAccountNumber) === normalizedAcct && r.round === 'rebuy1'
    );
  };
  ...
  const getLastRegisteredPlayer = () => {
    ...
    const playerTransactions = contextRegistrations.filter(r => r.playerAccountNumber === lastReg.playerAccountNumber);
    const purchases = [];
  ...
  const searchPlayer = () => {
    ...
    if (player) {
      if (activeTab === 'post-registration' && selectedRound !== 'round1' && !isRegisteredRound1(player.PlayerAccountNumber)) {
        alert('That player is not registered in ROUND 1 yet. Register them first before checking in to other rounds.');
        return;
      }
  ...
  const handleNewPlayerSave = (newPlayer, registerForRound1, registrationData) => {
    ...
    if (registerForRound1) {
      ...
      const registration = {
        ...
        tableNumber: null, 
        seatNumber: null,
        seatPreferences: registrationData.seatPreferences
      };
  ...
  const getButtonText = () => {
    ...
    if (isRebuyRound) {
      return addMulligan ? 'Check In and Add Mulligan' : 'Check In';
    } else {
      return addMulligan ? 'Check In and Add Mulligan' : 'Check In';
    }
  };
  ...
  const handleRegistration = () => {
    ...
    const isRebuyRound = currentRoundInfo?.isRebuy || false;
    const requiresPayment = (activeTab === 'registration') || isRebuyRound;
    
    if (requiresPayment) {
      if (!paymentType) {
        const roundName = currentRoundInfo ? currentRoundInfo.name : 'this round';
        alert(`Please select a payment type for ${roundName} before proceeding.`);
        return;
      }
      ...
      if (splitPayment && !checkForZeroAmounts(paymentAmount, paymentAmount2, 'payment')) {
        return;
      }
      
      let required = isRebuyRound ? currentTournament.rebuyCost : currentTournament.entryCost;
    ...
    if (addMulligan) {
      ...
      if (!amountsMatch(currentTournament.mulliganCost, mulliganAmount, splitMulliganPayment ? mulliganAmount2 : 0)) {
        alert(`Mulligan payments must add up to ${currentTournament.mulliganCost}.`);
        return;
      }
    }
    ...
    if (requiresPayment) {
      if (paymentType === 'Comp') {
        ...
        const total = (parseInt(paymentAmount) || 0) + (splitPayment ? (parseInt(paymentAmount2) || 0) : 0);
        eventType = isRebuyRound ? `REBUY ${total}` : `PAY $${total}`;
      }
    }
    ...
    const baseReg = {
      ...
      eventName: selectedEvent,
      eventType: eventType,
      paymentType: requiresPayment ? paymentType : null,
      paymentAmount: requiresPayment ? (parseInt(paymentAmount) || 0) : 0,
      paymentType2: (requiresPayment && splitPayment) ? paymentType2 : null,
      paymentAmount2: (requiresPayment && splitPayment) ? (parseInt(paymentAmount2) || 0) : null,
      registrationDate: new Date().toISOString(),
      host: host,
      comment: comments,
      round: roundKey,
  ...
  return (
    <div className="container">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%' }}>
          <div>
            ...
            <p className="page-subtitle">
              {currentTournamentRegistrations.filter(r => r.round === 'round1' && !r.isMulligan).length} total registrations, Employee: {employee}
            </p>
          </div>
      ...
      {showSearchBar && (
        <SearchBar searchValue={searchAccount} onSearchChange={setSearchAccount} onSearch={searchPlayer} placeholder="Enter 14-digit Player Account Number"/>
      )}
        ...
        <div>
          ...
          {activeTab === 'registration' && (
            <div className="form-group">
              ...
              <input type="text" value={host} onChange={(e) => setHost(UC(e.target.value))} className="input-field" placeholder="Enter host name"/>
            </div>
          )}
          <div className="form-group">
            <label className="mb-2">Comments</label>
            <textarea value={comments} onChange={(e) => setComments(UC(e.target.value))} className="textarea-field" rows="3"/>
          </div>
